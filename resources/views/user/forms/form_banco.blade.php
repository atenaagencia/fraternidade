...
"label": "Banco Keb Hana do Brasil S. A."
  ...
  $b = json_decode($lista);
  foreach ($b as $key) {
  $banco[] = [$key->value.' -'.$key->label => $key->value.' -'.$key->label];
  }
...
{!! Form::open(['route'=>'banco.store','class'=>'user']) !!}
@else
{!! Form::model($dadosBancario,['class'=>'user','route' => ['banco.update',$dadosBancario->id]]) !!}
@method('PUT')
@endif
...
<div class="form-group row mt-3">
    ...
    <div class="col-sm-6 mt-1 text-left">
        ...
        <select name="banco" id="" class="form-control form-control-user" required>
            ...
            @endif
            @foreach($b as $key)
            <option value="{{$key->value.' -'.$key->label}}" {{(isset($dadosBancario->banco) == $key->value.' -'.$key->label)? 'selected': ''}} se>{{$key->value.' -'.$key->label}}</option>
